...
+ (void)initialize
{
  ...
  if (self == ASDisplayNode.class) {
    ...
    class_addMethod(self, @selector(hierarcHYSwiftSignalKitDidFinish), noArgsImp, "v@:");
...
- (void)_pendingNodeDidDisplay:(ASDisplayNode *)node
{
  ...
  if (_pendingDisplayNodes.isEmpty) {
    ...
    [self hierarcHYSwiftSignalKitDidFinish];
    [self enumerateInterfaceStateDelegates:^(id<ASInterfaceStateDelegate> delegate) {
      [delegate hierarcHYSwiftSignalKitDidFinish];
    }];
