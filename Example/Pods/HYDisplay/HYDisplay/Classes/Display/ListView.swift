...
open class ListView: ASDisplayNode, ASScrollViewDelegate, ASGestureRecognizerDelegate {
    ...
    private var itemReorderingTimer: HYSwiftSignalKit.Timer?
    ...
    private func checkItemReordering(force: Bool = false) {
        ...
        if !force {
            self.itemReorderingTimer = HYSwiftSignalKit.Timer(timeout: 0.025, repeat: false, completion: { [weak self] in
