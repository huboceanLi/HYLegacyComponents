...
public final class ListViewReorderingGestureRecognizer: UIGestureRecognizer {
    ...
    private var longTapTimer: HYSwiftSignalKit.Timer?
    private var longPressTimer: HYSwiftSignalKit.Timer?
    ...
    private func startLongTapTimer() {
        self.longTapTimer?.invalidate()
        let longTapTimer = HYSwiftSignalKit.Timer(timeout: 0.25, repeat: false, completion: { [weak self] in
    ...
    private func startLongPressTimer() {
        self.longPressTimer?.invalidate()
        let longPressTimer = HYSwiftSignalKit.Timer(timeout: 0.6, repeat: false, completion: { [weak self] in
